...
def check_module_versions() -> None:
    ...
    gym_check = '(\u2713)' if LooseVersion(gym.__version__) >= LooseVersion('0.23.1') else '(\u2717)'
    pygame_check = '(\u2713)' if LooseVersion(pygame.__version__) >= LooseVersion('2.1.2') else '(\u2717)'
    ...
    print(f'Installed pygame version: {pygame.__version__} {pygame_check}')
...
def animate_environment_search(frames: Sequence[Dict[str, Any]], verbose: bool = False, delay: float = 0.01) -> None:
    """
    Create and display animation from specified OpenAI Gym environment <ansi> frames. This function was taken from [1]
    and slightly adapted.

    [1] https://www.learndatasci.com/tutorials/reinforcement-q-learning-scratch-python-openai-gym/; accessed: 12.05.2020

    :param frames: sequence of frames to animate
    :param verbose: print additional frame information
    :param delay: time delay in seconds between each frame
    """
    for frame_index, frame_data in enumerate(frames):
        display.clear_output(wait=True)
        print(frame_data[r'frame'])

        if verbose:
            print(f'Step No.: {frame_index + 1}'
                  f'\nState ID: {frame_data[r"state"]}'
                  f'\nAction ID: {frame_data[r"action"]}'
                  f'\nReward: {frame_data[r"reward"]}')
        time.sleep(delay)
