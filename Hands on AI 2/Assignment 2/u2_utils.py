...
def load_mnist() -> pd.DataFrame:
    ...
    """
    mnist_data = datasets.fetch_openml(name=r'mnist_784', as_frame=True)
...
def plot_points_2d(data: pd.DataFrame, target_column: Optional[str] = None, targets: Sequence = None, legend: bool = True, **kwargs) -> None:
    """
...
def plot_decision_boundaries(data: pd.DataFrame, classifier: ClassifierMixin, target_column: Optional[str] = None,
    ...
    if legend:
        if hasattr(cmap, r'colors'):
            cmap = cmap.colors
        sns.scatterplot(x=data_stripped.iloc[0, 0], y=data_stripped.iloc[0, 1], hue=hue, palette=cmap, alpha=0.0,
                        legend=r'full')
