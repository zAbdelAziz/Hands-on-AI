...
educational use only. Any reproduction of this material, no matter whether as a whole or in parts, no matter
whether in printed or in electronic form, requires explicit prior acceptance of the authors.
"""
import random
import sys
...
def test_network(model: torch.nn.Module, data_loader: torch.utils.data.DataLoader,
    ...
    with torch.no_grad():
        for data, target in data_loader:
            ...
            # Compute accuracy of current model w.r.t. the specified data set.
            pred = output.argmax(dim=1).view(-1).long()
            num_correct += (pred == target.view(-1)).sum().item()
...
class LatchSequenceSet(torch.utils.data.Dataset):
    ...
    @property
    def resort(self) -> r'LatchSequenceSet':
        sorted_data = deepcopy(self)
        for sequence, position in zip(sorted_data.__data, sorted_data.__positions):
            triple_source = sequence[:min(3, (position - 2))].clone()
            triple_target = sequence[position - 2:position + 1].clone()
            sequence[:3] = triple_target
            sequence[position - min(2, (position - 3)):position + 1] = triple_source
        sorted_data.__positions = torch.zeros_like(sorted_data.__positions) + 2
        return sorted_data
