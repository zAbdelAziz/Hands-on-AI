# -*- coding: utf-8 -*-
"""
Authors: Brandstetter, Schäfl, Schlüter, Schörgenhumer
Additional authors of previous version: Mitterecker, Ramsauer, Rumetshofer, Winter, Patil
Date: 13-12-2021
...
def run_gradient_descent(model: torch.nn.Module, loss: Callable[[torch.Tensor, torch.Tensor], torch.Tensor],
    ...
    assert len(training_set.dataset) == len(training_set) * training_set.batch_size
    errors = []
    valid_errors = []
    for epoch in range(iterations):
        ...
        for inputs, targets in training_set:
            ...
            optimizer.step()
            optimizer.zero_grad()
            pbar.update(1 if show_batch_progress else training_set.batch_size)
        errors[-1] /= len(training_set)
        print(f'Epoch {epoch + 1} finished with training loss: {errors[-1]}')
...
def plot_input_weights(model: Union[torch.nn.Linear, torch.nn.Sequential], input_shape: Tuple[int, int],
    ...
    :param ncols: Number of columns of the plot (number of rows are determined automatically).
